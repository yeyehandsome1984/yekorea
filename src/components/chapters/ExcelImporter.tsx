      ...
      reader.onload = async (e) => {
        try {
          ...
          const uniqueWords = words.filter(newWord => {
            const isDuplicate = existingWords.some(
              existingWord => existingWord.word.toLowerCase() === newWord.word.toLowerCase()
            );
            if (isDuplicate) {
            ...
            {previewData.map((item: any, idx) => (
              <div 
                ...
                className={`flex flex-col p-2 rounded-sm ${
                  item.duplicate ? 'bg-red-50 border border-red-200' : 'bg-gray-50'
                }`}
              >
                ...
                {item.example && (
                  <span className="text-sm italic text-gray-600">"{item.example}"</span>
                )}
                {item.duplicate && (
                  <span className="text-xs text-red-600 mt-1">
                    This word already exists in the chapter and will be skipped.
                  </span>
                )}
              </div>
            ))}
